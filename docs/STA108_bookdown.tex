...
\tightlist
\item
  Fourth week: Least squares estimation: optimization (Slides 22-27) Sum
  of squares: (Slides 28 - 30) + add projection in this discussion
\end{itemize}
...
\KeywordTok{solve}\NormalTok{(Xmat1) }\CommentTok{# what does the error message say?}


\NormalTok{Xmat12=Xmat1}\OperatorTok{%*%}\KeywordTok{t}\NormalTok{(Xmat2);}

\NormalTok{Xmat12.inv=}\KeywordTok{solve}\NormalTok{(Xmat12);}

\NormalTok{Xmat12.inv}\OperatorTok{%*%}\NormalTok{Xmat12}
\NormalTok{Xmat12}\OperatorTok{%*%}\NormalTok{Xmat12.inv}



\KeywordTok{svd}\NormalTok{(Xmat1)  }\CommentTok{# singular value decomposition of Xmat1}
\NormalTok{### Q: what is the SVD?}
\NormalTok{### Try ?svd}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{?array }\CommentTok{# check for R documentation on array}
